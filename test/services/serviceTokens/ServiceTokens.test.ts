...
describe('test ServiceTokens', () => {
  ...
  describe('test list', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=deleniti&config=minima')
        .reply(200, { data: {} });
      return sdk.serviceTokens
        .list('deleniti', 'minima')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=voluptates&config=odit')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=dicta&config=rerum')
        .reply(404, { data: {} });
      return expect(async () => await sdk.serviceTokens.list('dicta', 'rerum')).rejects.toThrow();
    });
  });
